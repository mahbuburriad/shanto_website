...
import {
  Menu, X, Mail, Phone, Download, Award, BookOpen,
  GraduationCap, BriefcaseBusiness, FileText, ListChecks,
  Newspaper, Home as HomeIcon, Link as LinkIcon, User
} from "lucide-react";
import "./App.css";
...
const SITE = {
  ...
  headshot: new URL("./assets/headshot.jpg", import.meta.url).href,
  cvUrl: "/CV_of_Shanto.pdf",
  links: [
...
const EXPERIENCE = [
  ...
  { role: "Teaching Assistant", org: "Department of Economics, Texas Tech University", dates: "Fall 2023 – Spring 2025",
    bullets: [
...
const CONFERENCES = [
  { authors: "Shanto, P. A., Hasan, M.",
    title: "Unraveling the Relationship Between Governance and Ecological Footprint: A System GMM Analysis of Global Data",
...
const POSTERS = [
  { authors: "Shanto, P. A., Hasan, M.", title: "The Impact of Income Inequality on Ecological Footprint",
    venue: "23rd Annual Graduate Student Research Poster Competition", place: "Texas Tech University, Lubbock, TX", dates: "Mar 8, 2024" },
  { authors: "Shanto, P. A., Hasan, M.",
    title: "Unraveling the Relationship Between Governance and Ecological Footprint: A System GMM Analysis of Global Data",
    venue: "Illinois Geographical Society Meeting", place: "Fort Madison, Iowa", dates: "Apr 25–28, 2024" }
];
          ...
          <nav className="hidden md:flex gap-4">
            {links.map(({ label, to }) => (
              <NavLink key={to} to={to}
                className={({ isActive }) =>
                  `text-sm px-2 py-1 rounded-xl ${isActive ? 'bg-gray-900 text-white' : 'text-gray-700 hover:bg-gray-100'}`}>
                {label}
              </NavLink>
            ))}
          </nav>
          <div className="flex items-center gap-3">
            <a href={SITE.cvUrl} className="hidden sm:inline-flex items-center gap-2 rounded-2xl border px-3 py-1.5 text-sm hover:shadow">
              <Download className="w-4 h-4"/> Download CV
            </a>
            <button className="md:hidden rounded-xl p-2 border" onClick={() => setOpen(!open)} aria-label="Toggle menu">
        ...
        {open && (
          <div className="md:hidden border-t bg-white">
            <div className="max-w-6xl mx-auto px-4 py-3 grid gap-2">
              {links.map(({ label, to }) => (
                <NavLink key={to} to={to} className="py-2 text-sm" onClick={() => setOpen(false)}>
                  {label}
                </NavLink>
              ))}
            </div>
      ...
      <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-3 gap-8 items-center">
        <div className="md:col-span-2">
          ...
          <p className="mt-4 text-gray-700">
            I am an applied economist interested in how policy, markets, and risk shape sustainability—especially in energy and environmental domains.
            My current research focuses on ecological footprints, governance, and the economics of growth & trade.
          </p>
...
function Experience() {
  return (
    <section className="py-12">
      <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-2 gap-4">
        {EXPERIENCE.map((x) => (
          <div key={x.role + x.org} className="card">
...
function Awards() {
  return (
    <section className="py-12">
      <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-2 gap-4">
        {AWARDS.map((a) => (
          <div key={a.name} className="card flex items-start gap-3">
